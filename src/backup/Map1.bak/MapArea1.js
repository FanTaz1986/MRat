...
import { playFootstepLoop, stopFootstepLoop } from "./soundsmap1";
...
import { DebugMenuOverlay } from "../Map0/debug0";
...
export default function MapArea() {
  ...
  useEffect(() => {
    function handleDebugKeys(e) {
      ...
      if (e.key === "2" && portal) {
        ...
        setLoadMap2(true); // Instantly load Map2 after teleporting to portal
      }
    }
  ...
  // 4x4 center: only A
        ...
        for (let p = 0; p < props.length && propCount < 20; p++) {
          ...
          tiles.push(
            <PropImg
              ...
              alt={"propA"}
              style={{
                width: isBush ? 192 : isTree ? 288 : 96,
                height: isBush ? 192 : isTree ? 288 : 96,
                position: "absolute",
      ...
      function tileOverlapsPortalArea(tileX, tileY, MAP_SIZE, portal, radius = 220 * 5) {
